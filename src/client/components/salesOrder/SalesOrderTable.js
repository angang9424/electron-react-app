...
function SalesOrderTable({pos = [], pages = 0, rowsPerPages = 0, editData, deleteData}) {
		...
		<Card>
			<Box sx={{ overflowX: 'auto' }}>
				<Table sx={{ minWidth: '800px' }}>
					...
					<TableBody>
						{paginatedBook.map((so) => {
							// const isSelected = selected?.has(row.id);

							return (
								<TableRow hover key={so.id} onClick={() => navigate('/sales_order_details', {state: { so_id: so.id }})}>
									<TableCell>{ so.id }</TableCell>
									<TableCell>{ dayjs(so.date).format('MMM D, YYYY') }</TableCell>
